...
const FourthSection = () => {
  ...
  const fetchReviews = () => {
    axios
      .get(`${process.env.REACT_APP_BACKEND_URL}/api/reviews`)
  ...
  const handleSubmitReview = () => {
    const token = localStorage.getItem("token");
    if (!token || !newReview.trim() || rating === 0) {
      toast.error("Please provide text and a star rating.");
      return;
    }
